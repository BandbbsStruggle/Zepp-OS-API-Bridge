/* __$$hmAppManager$$__.currentApp.current.module = DeviceRuntimeCore.Page({
    build() { */
        export const widget = {
          ...
          export function createWidget(widgetType, params) {
      ...
      this.previousMileage2 = savedPreviousMileage2;
        ...
        hmApp.gotoHome()
          ...
          export function showToast (options) {
                     ...
                     function str2ab(str) {
                      ...
                      switch(hmSetting.getDateFormat()) {
                        case 0: value = "DATE_FORMAT_YMD" 
                        break;
                        case 1: value = "DATE_FORMAT_DMY" 
                        break;
                        case 2: value = "DATE_FORMAT_MDY" 
                        break;
                      }
                      return value
                     }   
                     function getDistanceUnit () {
                      ...
                      switch(hmSetting.getMileageUnit()) {
                        case 0: value = "DISTANCE_UNIT_METRIC" 
                        break;
                        case 1: value = "DISTANCE_UNIT_IMPERIAL" 
                        break;
                      }
                      return value
                     }   
                     ...
                     export function getSleepTarget () {
                     ...
                     export function getWeightUnit () {
                      ...
                      switch(hmSetting.getWeightUnit()) {
                        case 0: value = "WEIGHT_UNIT_KILOGRAM" 
                        break;
                        case 1: value = "WEIGHT_UNIT_JIN" 
                        break;
                        case 2: value = "WEIGHT_UNIT_POUND" 
                        break;
                        case 3: value = "WEIGHT_UNIT_STONE" 
                        break;
                      }
                      return value
                     }   
                     export function getAutoBrightness () {
                      return hmSetting.getScreenAutoBright()
                     } 
                     export function getBrightness () {
                      return hmSetting.getBrightness()
                     } 
                     export function setAutoBrightness(page) {
                      hmSetting.setScreenAutoBright(page.autoBright)
                    }
                    export function setBrightness(page) {
                    ...
                    let value
                      if (typeof hmDevice !== 'undefined') {
                        switch(hmSetting.getDeviceInfo().screenShape) {
                          case 0: value = "SCREEN_SHAPE_SQUARE" 
                          break;
                          case 1: value = "SCREEN_SHAPE_ROUND" 
                          break;
                        }
                        deviceInfo = {
                    ...
                    export function getProfile() {
                      ...
                      switch(hmSetting.getUserData().gender) {
                        case 0: value = GENDER_MALE
                        break;
                        case 1: value = GENDER_FEMALE
                        break;
                        case 2: value = GENDER_UNSPECIFIED
                        break;
                      }
                      if (typeof hmDevice !== 'undefined') {
                        getProfile = {
          ...
          return null;
            ...
            export function statAssetsSync(filename) {
...
export function writeFileSync(filename) {
...
export function openSync(filename) {
  return hmFS.open(filename.path, filename.flag)
}
export function closeSync({path}) {
  return hmFS.close(path);
}
export function mkdirSync({ filename}) {
...
export function getScene () {
                      let value
                      switch(hmSetting.getWeightUnit()) {
                        case hmSetting.screen_type.APP: value = "SCENE_APP" 
                        break;
                        case hmSetting.screen_type.WATCHFACE: value = "SCENE_WATCHFACE" 
                        break;
                        case hmSetting.screen_type.SETTINGS: value = "SCENE_SETTINGS" 
                        break;
                        case hmSetting.screen_type.AOD: value = "SCENE_AOD" 
                        break;
                      }
                      return value
                    }
...
export function animateWidget(widgetId, options) {
  ...
  const { anim_steps, anim_fps } = options;
  ...
  function updateAnimation(currentTime) {
    ...
    const progress = Math.min(elapsed / anim_steps[currentStep].anim_duration, 1);

    const animatedValue = interpolate(
      anim_steps[currentStep].anim_from,
      anim_steps[currentStep].anim_to,
      progress,
      linear
    );

    setPropertyValue(widget, anim_steps[currentStep].anim_prop, animatedValue);

    if (progress >= 1) {
      currentStep++;
      if (currentStep < anim_steps.length) {
...
function onAnimationComplete(animId) {
  delete animations[animId];
}

function setPropertyValue(widget, property, value) {
  switch (property) {
    case 'x':
      widget.style.left = value;
      break;
    case 'y':
      widget.style.top = value;
      break;
    // 添加其他属性设置逻辑
    default:
      break;
...
function interpolate(startValue, endValue, progress, easing) {
  // 在这里实现你想要的属性插值逻辑
  // 例如，可以使用 easing 函数进行插值计算

  return easing(progress) * (endValue - startValue) + startValue;
}
...
function setAnimationStatus(animId, status) {
  const animation = animations[animId];
  if (!animation) {
    console.error(`Animation with id "${animId}" not found`);
    return;
  }

  animation.anim_status = status;
  if (status === 'paused') {
    cancelAnimationFrame(animation.animationId);
  } else if (status === 'running' && !animation.animationId) {
    const widget = document.getElementById(animation.widgetId);
    if (widget) {
      const options = {
        anim_steps: animation.anim_steps,
        anim_fps: animation.anim_fps,
      };
      animateWidget(animation.widgetId, options);
    }
  }
}

function getAnimationStatus(animId) {
  const animation = animations[animId];
  if (!animation) {
    console.error(`Animation with id "${animId}" not found`);
    return '';
...
function startTimerLoop() {
  setInterval(registerTimerCallbacks, 10);
} */
/*          const textWidget = createWidget(widget.TEXT, {
            ...
            text_i18n: {
              'en-US': 'Zeppos API Bridge Multilingual test text',
              'zh-CN': 'Zeppos API Bridge 多语言测试文本'
            }
          })
...
onInit(){this.build()},
onDestory() {
if (typeof(num) == Number) hmSetting.setBrightScreenCancel()
